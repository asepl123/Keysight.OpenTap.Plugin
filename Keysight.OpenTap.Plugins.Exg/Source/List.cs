...
{
    [Display("List", Group: "Keysight.OpenTap.Plugins.Exg.Source", Description: "Insert a description here")]
    public class List : TestStep
    {
        #region Settings
        ...
        {
            QueryCurrentSPoint = MyInst.ScpiQuery<System.Int32>(Scpi.Format(":SOURce:LIST:CPOint?"), true);
            ...
            MyInst.ScpiCommand(":SOURce:LIST:FREQuency {0}", ListFrequency);
            QueryListDirection = MyInst.ScpiQuery<System.Double[]>(Scpi.Format(":SOURce:LIST:FREQuency?"), true);
            MyInst.ScpiCommand(":SOURce:LIST:POWer {0}", ListPower);
            MyInst.ScpiCommand(":SOURce:LIST:TYPE {0}", SweepType);
        }
    }
}
